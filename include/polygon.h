...
/**
 * Computes the center point of a polygon, computed from its vertices.
 *
 * @param polygon the list of vertices that make up the polygon,
 * listed in a counterclockwise direction. There is an edge between
 * each pair of consecutive vertices, plus one between the first and last.
...
/**
 * @brief Stretches a polygon by a given factor in the x direction
 *
 */
void polygon_stretch_x(list_t *polygon, double factor);

/**
 * @brief Reflects a polygon across a given line x=x0
 *
 */
void polygon_reflect_x(list_t *polygon, double x0);

/**
 * @brief Reflects a polygon across a given line y=y0
 *
 */
