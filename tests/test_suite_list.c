...
void test_list_small() {
  ...
  *v = VEC_ZERO;
  ...
  assert(vec_equal(*(vector_t *)v, (vector_t){4, 4}));
  free(v);
  v = list_remove(l, list_size(l) - 1);
  assert(vec_equal(*(vector_t *)v, (vector_t){3, 3}));
  free(v);
  assert(list_size(l) == 3);
...
void test_list_large_add_remove() {
  ...
  for (size_t i = 1; i <= LARGE_SIZE; i++) {
    ...
    assert(vec_equal(*(vector_t *)v, (vector_t){value, value}));
    free(v);
  }
...
void remove_from_empty(void *l) { list_remove((list_t *)l, list_size(l)); }
void test_empty_remove() {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    assert(vec_equal(*(vector_t *)list_v, v));
    free(list_v);
  }
